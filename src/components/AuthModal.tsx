...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // Google Social Sign In Trigger (Triggers phone's Google prompt or account chooser)
  const handleGoogleSignIn = async () => {
    setErrorMsg('');
    setSuccessMsg('');

    // Clear any previously set Google fields so nothing is auto-filled
    setGoogleEmail('');
    setGoogleName('');
    setGoogleRole(accountType);
    setGoogleBizName(businessName);

    // 1. If Google OAuth 2.0 / GIS is configured, trigger Google OAuth Account Chooser popup
    if (isGoogleClientConfigured() && typeof window !== 'undefined') {
      ...
      triggerGoogleOAuthFlow(
        ...
        (_err) => {
          setGoogleLoading(false);
          // Show interactive Google Account Chooser screen for device accounts / manual selection
    ...
    // 2. Interactive Google Account Chooser Screen
    setAuthMode('google_prompt');
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ------------------------------------------------------------------ */}
        {/* VIEW 3: GOOGLE ACCOUNT CHOOSER (SELECT REGISTERED OR ENTER ACCOUNT)*/}
        {/* ------------------------------------------------------------------ */}
        {authMode === 'google_prompt' && (
          <div className="space-y-5">
            ...
            {/* List Registered Accounts on this Device if Available */}
            {savedAccounts.length > 0 && (
              <div className="space-y-2">
                <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-1">
                  Registered Google Accounts on this Device:
                </div>
                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-0.5">
                  {savedAccounts.map((acc) => {
                    const isBiz = acc.role === 'business_owner' || acc.role === 'verified_owner';
                    return (
                      <button
                        key={acc.id || acc.email}
                        type="button"
                        onClick={() => handleExecuteGoogleLogin(acc.email, acc.name, isBiz ? 'business_owner' : 'customer', acc.businessName)}
                        disabled={googleLoading}
                        className="w-full p-3 rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 hover:border-blue-500 hover:bg-blue-50/40 dark:hover:bg-slate-800 text-left flex items-center justify-between group transition-all cursor-pointer shadow-xs"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-blue-600 text-white font-bold text-xs flex items-center justify-center overflow-hidden shrink-0">
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-800">
              <div className="text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2.5 px-1">
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-2 ${
                      googleRole === 'business_owner'
                        ? 'border-amber-600 bg-amber-50/60 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400'
                        : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'
                    }`}
                  >
